  ...
  const questions = [
    {
      ...
      type: 'input',
      name: 'gulp',
      message: '[General] Where do you want to store gulp tasks for the project (relative to default path)?',
      default: config.getOptionValue('gulp'),
      validate (value) {
        if (value.length) {
          return true
        }
      ...
      type: 'confirm',
      name: 'gzip[run]',
      message: '[COMPRESS] Do you use compress (gzip) all assets?',
