  ...
  const questions = [
    {
      ...
      type: 'confirm',
      name: 'favicons[run]',
      message: '[FAVICONS] Do you want to run favicon tasks?',
      default: config.getOptionValue('favicons.run')
    }, {
      type: 'input',
      name: 'favicons[src]',
      message: '[FAVICONS] Where is the folder with favicons (relative to default source path)?',
      default: config.getOptionValue('favicons.src'),
      validate (value) {
        if (value.length) {
          return true
        }
      ...
      when (answers) {
        return answers.favicons.run === true
      }
    }, {
      type: 'input',
      name: 'favicons[dist]',
      message: '[FAVICONS] Where do you want to store favicons (relative to default destination path)?',
      default: config.getOptionValue('favicons.dist'),
      when (answers) {
        return answers.favicons.run === true
      }
    }, {
      ...
      default: config.getOptionValue('penthouse.run')
    }, {
      type: 'confirm',
      name: 'gzip[run]',
