...
const copyFile = (filePath, destPath = './', destFile) => {
  try {
    ...
    return ncp(path.join(__dirname, '..', _s(filePath)), `${slash(destPath)}${destFile}`, { clobber: clb }, log.message);
  } catch (err) {
    log.message(err);
...
const saveFile = (filePath, content) => {
  try {
    fs.writeFileSync(_(filePath), JSON.stringify(content, null, 2), 'utf-8');
